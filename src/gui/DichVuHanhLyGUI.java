...
public class DichVuHanhLyGUI extends JPanel {
    ...
    public DichVuHanhLyGUI(DatVeSession session) {
        ...
        this.tongTienTamTinh = session.tongTienVe; 
        initComponents();
    }
    ...
    private void initComponents() {
        ...
        pnlHanhLy.setBorder(BorderFactory.createTitledBorder(BorderFactory.createLineBorder(new Color(200, 200, 200), 1, true), "1. HÀNH LÝ KÝ GỬI", TitledBorder.LEFT, TitledBorder.TOP, new Font("Segoe UI", Font.BOLD, 18), PRIMARY_COLOR)); 
        ...
        if (session.danhSachHanhKhach != null) {
            for (int i = 0; i < session.danhSachHanhKhach.size(); i++) {
                ...
                JLabel lblKhach = new JLabel("Hành khách " + (i + 1) + ": "); 
                lblKhach.setPreferredSize(new Dimension(150, 40)); 
                lblKhach.setFont(FONT_LABEL);
                
                // SIZE BỰ CHO COMBOBOX HÀNH LÝ
                JComboBox<BaggageItem> cbo = new JComboBox<>(optsHanhLy); 
                cbo.setPreferredSize(new Dimension(400, 45));
                cbo.setFont(new Font("Segoe UI", Font.PLAIN, 16));
        ...
        JPanel hlWrapper = new JPanel(new FlowLayout(FlowLayout.CENTER, 0, 0)); 
        hlWrapper.setOpaque(false); 
        pnlHanhLy.setPreferredSize(new Dimension(1000, session.danhSachHanhKhach != null ? session.danhSachHanhKhach.size() * 65 + 60 : 150));
        hlWrapper.add(pnlHanhLy);
        
        scrollContent.add(hlWrapper); 
        ...
        pnlDichVu.setBorder(BorderFactory.createTitledBorder(BorderFactory.createLineBorder(new Color(200, 200, 200), 1, true), "2. DỊCH VỤ BỔ SUNG", TitledBorder.LEFT, TitledBorder.TOP, new Font("Segoe UI", Font.BOLD, 18), PRIMARY_COLOR)); 
        pnlDichVu.setPreferredSize(new Dimension(1000, 250));
        ...
        chkContainer.setBorder(new EmptyBorder(15,30,15,30));
        
        try { listDichVuDB = dichVuBUS.docDanhSachDichVu(); } catch(Exception e) { listDichVuDB = new ArrayList<>(); }
        ...
        for (DichVuBoSung dv : listDichVuDB) {
            ...
            chk.setFont(new Font("Segoe UI", Font.PLAIN, 18)); // CHỮ TO RÕ RÀNG
            ...
            chkContainer.add(Box.createRigidArea(new Dimension(0, 20)));
        }
        pnlDichVu.add(chkContainer, BorderLayout.CENTER); 
        
        JPanel dvWrapper = new JPanel(new FlowLayout(FlowLayout.CENTER, 0, 0)); 
        dvWrapper.setOpaque(false); 
        dvWrapper.add(pnlDichVu);
        scrollContent.add(dvWrapper);

        // BỌC VÀO TOP ĐỂ LUÔN NẰM SÁT TRÊN
    ...
    private JPanel createStickyFooter() {
        ...
        btnQuayLai.setBackground(Color.WHITE); btnQuayLai.setPreferredSize(new Dimension(250, 45)); btnQuayLai.setFont(new Font("Segoe UI", Font.BOLD, 16)); btnQuayLai.setBorder(BorderFactory.createLineBorder(new Color(200, 200, 200), 2)); btnQuayLai.setCursor(new Cursor(Cursor.HAND_CURSOR));
        ...
        JPanel pnlTotalText = new JPanel(new GridLayout(2, 1)); pnlTotalText.setOpaque(false); pnlTotalText.add(new JLabel("Tổng tiền tạm tính:", SwingConstants.RIGHT)); 
        NumberFormat vn = NumberFormat.getInstance(new Locale("vi", "VN"));
        lblFooterTotal = new JLabel(vn.format(tongTienTamTinh) + " VNĐ", SwingConstants.RIGHT); lblFooterTotal.setFont(new Font("Segoe UI", Font.BOLD, 24)); pnlTotalText.add(lblFooterTotal);
        
        JButton btnNext = new JButton("Đi tiếp"); btnNext.setBackground(new Color(255, 193, 7)); btnNext.setPreferredSize(new Dimension(250, 45)); btnNext.setFont(new Font("Segoe UI", Font.BOLD, 16)); btnNext.setForeground(new Color(18, 32, 64)); btnNext.setCursor(new Cursor(Cursor.HAND_CURSOR));
    ...
    private void processTiepTuc() {
        BigDecimal tongTienDichVu = BigDecimal.ZERO; session.danhSachHanhLy.clear(); 
        for (int i = 0; i < listCboHanhLy.size(); i++) {
            BaggageItem selected = (BaggageItem) listCboHanhLy.get(i).getSelectedItem();
            if (selected.kg > 0) {
                tongTienDichVu = tongTienDichVu.add(selected.price); HanhLy hl = new HanhLy(); hl.setSoKg(new BigDecimal(selected.kg)); hl.setGiaTien(selected.price); hl.setGhiChu("Khách: " + session.danhSachHanhKhach.get(i).getHoTen()); session.danhSachHanhLy.add(hl);
            }
        }
        for (int i = 0; i < chkListDichVu.size(); i++) { if (chkListDichVu.get(i).isSelected()) { tongTienDichVu = tongTienDichVu.add(listDichVuDB.get(i).getDonGia()); } }
